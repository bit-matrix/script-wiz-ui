...
import {VM_NETWORK, VM_NETWORK_VERSION } from '@script-wiz/lib';
...
const CompileModal = ({ scriptWiz, compileModalData, showCompileModal }) => {
  const [keyPath, setKeyPath] = useState(KeyPath.UNKNOWN);
  const [tapleafVersion, setTapleafVersion] = useState(TapleafVersion.DEFAULT);
  const [pubKeyInput, setPubKeyInput] = useState('');
  const [tapleafInput, setTapleafInput] = useState('');
  const [tweakedResult, setTweakedResult] = useState({ tweak: '', scriptPubkey: '', address: { mainnet: '', testnet: '' } });
  const [segwitAddress, setSegwitAddress] = useState();

  const pubkeyDefaultValue= '1dae61a4a8f841952be3a511502d4f56e889ffa0685aa0098773ea2d4309f624';
  const tapleafDefaultValue= scriptWiz.vm.network === VM_NETWORK.LIQUID ? '0xc4' : '0xc0';

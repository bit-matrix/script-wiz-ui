...
const initialLineStackDataListArray = [];
const initialLastStackDataList = [];

const ScriptEditor = ({ scriptWiz }) => {
  const [witnessScriptErrorMessage, setWitnessScriptErrorMessage] = useState();
  const [stackElementsErrorMessage, setStackElementsErrorMessage] = useState();
  const [witnessScriptLineStackListArray, setWitnessScriptLineStackListArray] = useState(initialLineStackDataListArray);
  const [stackElementsLineStackListArray, setStackElementsLineStackListArray] = useState(initialLineStackDataListArray);
  const [failedLineNumber, setFailedLineNumber] = useState();
  const [witnessScriptLines, setWitnessScriptLines] = useState();
  const [stackElementsLines, setStackElementsLines] = useState();
  const [editorValues, setEditorValues] = useState([]);
  const [witnessScriptLastData, setWitnessScriptLastData] = useState(initialLastStackDataList);
  const [editorSplits, setEditorSplits] = useState({
    direction: 'row',
    first: {
      direction: 'column',
      first: 'input1',
  ...
  const [compileModalData, setCompileModalData] = useState({ show: false });
  ...
  const [witnessScriptTop, setWitnessScriptTop] = useState(0);
  const [stackElementsTop, setStackElementsTop] = useState(0);

  const timerRef = useRef(undefined);

  const { clearTxLocalData: clearTxLocalDataEx } = useLocalStorageData(LOCAL_STORAGE_OLD_KEY);
  const { getTxLocalData, setTxLocalData, clearTxLocalData } = useLocalStorageData(LOCAL_STORAGE_KEY);
  ...
  useEffect(() => {
    ...
    if (localStorageValue) {
      ...
      const localStorageObject = localStorageArray.find((lsa) => {
  ...
  const saveLocalStorageData = useCallback(() => {
    if (clearButtonVisibility) {
      ...
      if (currentLocalStorage && currentLocalStorage !== null) {
        ...
        const localStorageValue = [{ editorLines1: editorValues[0], editorLines2: editorValues[1], vm: scriptWiz.vm }];
  ...
  const clearLocalStorageData = () => {
    ...
    if (localStorageData) {
      ...
      const currentLocalStorageDataIndex = localStorageDataJSON.findIndex(
        (value) => value.vm.network === scriptWiz.vm.network && value.vm.ver === scriptWiz.vm.ver,
      );
  ...
  const parseInput = useCallback(
    (inputText, isWitnessElement = true) => {
  ...
  const stackElementsOnChange = (lines) => {
  ...
  const witnessScriptOnChange = (lines2) => {
  ...
  useEffect(() => {
    ...
    let hasError = false;
    const newLineStackDataListArray = [];
    let newLastStackDataList = [];
  ...
  const compileIonioArtifact = () => {
    ...
    let functionInputs = [];
    let constructorInputsValues = new Map();
    let require = [];

    const inputHexes = scriptWiz.stackDataList.inputHexes;
    const cleanInputHexes = inputHexes.filter((hex) => hex && hex.length > 0);
    ...
    cleanInputHexes.forEach((hex, index) => {
      const opcode = scriptWiz.opCodes.codeData(Number(`0x${hex}`));
      if (!opcode) return;
      if (!opcode.word) {
  ...
  const handleScrollFourth = (event) => {

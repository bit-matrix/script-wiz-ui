...
export const Helper = () => {
  ...
  const [convertedWizData, setConvertedWizData] = useState(initialState);
  const [convertType, setConvertType] = useState(CONVERT_TYPE.FROM_HEX);
  const [errorMessage, setErrorMessage] = useState(undefined);
  const [checkedLe, setCheckedLe] = useState();

  useEffect(() => {
    const getResults = (wizdataInput) => {
      let hexResult = '';
      let hexLeResult = '';
      let binResult = '';
      let binLeResult = '';
      ...
      <div className="helper-page-item">
        <Form>
          <div className="helper-input-text">
            ...
            <InputGroup inside>
              <Input className="helper-main-input" type="text" value={input} onChange={(value) => setInput(value.replace(/\s/g, ''))} />
              <InputGroup.Button
                onClick={() => {
                  setConvertedWizData(initialState);
                  setErrorMessage(undefined);
